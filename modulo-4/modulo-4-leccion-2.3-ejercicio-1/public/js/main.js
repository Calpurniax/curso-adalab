...
document.querySelector('.js-btn-get-users').addEventListener('click', () => {
  const filterByName = document.querySelector('.js-btn-user-filter-name');
  const filterByEmail = document.querySelector('.js-btn-user-filter-email');
  fetch(`http://localhost:3000/users?name=${filterByName.value}&email=${filterByEmail.value}`)
    .then(response => response.json())
    .then(responseData => {
      console.log('Server response:', responseData);
